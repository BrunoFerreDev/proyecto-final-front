    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
      <div class="flex flex-col gap-2">
        ...
        <p class="text--white text-lg max-w-2xl">
          Complete el formulario para registrar un nuevo integrante en la base
          de datos de la liga.
        </p>
      </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-4 flex flex-col gap-6">
        ...
        <div class="bg-surface-light p-6 rounded-lg border shadow-sm">
          ...
          <div class="flex flex-col gap-2">
            <label @click="
              () => {
                (activePersona = 'JUGADOR'), (selectedTipo = 'JUGADOR');
              }
            "
            ...
            <label @click="
              () => {
                (activePersona = 'ARBITRO'), (selectedTipo = 'ARBITRO');
              }
            "
              class="relative flex items-center p-3 rounded-xl border cursor-pointer has-[:checked]:bg-[#1f44f9]/10 has-[:checked]:border-[#1f44f9] transition-colors group">
            ...
            <label @click="
              () => {
                (activePersona = 'CT'), (selectedTipo = 'CT');
              }
            "
              class="relative flex items-center p-3 rounded-xl border cursor-pointer has-[:checked]:bg-[#1f44f9]/10 has-[:checked]:border-[#1f44f9] transition-colors group">
      ...
      <div class="lg:col-span-8">
        <form class="flex flex-col gap-6" @submit.prevent="nuevaPersona">
          ...
          <div class="bg-surface-light p-6 md:p-8 rounded-lg border shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <label class="flex flex-col gap-2">
                <span class="text-sm font-semibold text-[#1c1c0d]">Nombre <span class="text-red-500">*</span></span>
                <input
                  class="w-full rounded-xl border py-3 px-4 text-[#1c1c0d] placeholder-text-white/60 transition-shadow"
                  placeholder="Ej. Lionel Andrés" type="text" required v-model="personaNueva.nombre" />
              </label>
              <label class="flex flex-col gap-2">
                <span class="text-sm font-semibold text--[#1c1c0d]">Apellido <span class="text-red-500">*</span></span>
                <input
              ...
              <label class="flex flex-col gap-2 md:col-span-2">
                ...
                <input
                  class="w-full rounded-xl border focus:border-transparent py-3 px-4 text--[#1c1c0d] placeholder-text-white/60 transition-shadow"
                  placeholder="usuario@ejemplo.com" type="email" v-model="personaNueva.email" required />
              </label>
              <label class="flex flex-col gap-2 md:col-span-2">
                <span class="text-sm font-semibold text--[#1c1c0d]">Tel&eacute;fono</span>
                <input
                  class="w-full rounded-xl border focus:border-transparent py-3 px-4 text--[#1c1c0d] placeholder-text-white/60 transition-shadow"
                  placeholder="Ej. +54 9 11 2345-6789" type="tel" v-model="personaNueva.telefono" required />
              </label>
            </div>
          </div>
          ...
          <div class="bg-surface-light p-6 md:p-8 rounded-lg border shadow-sm relative overflow-hidden" v-if="
            activePersona == 'JUGADOR' || activePersona == 'CUERPO_TECNICO'
          ">
            <!-- Decorative bg element -->
            <div class="absolute -top-10 -right-10 size-40 bg-[#1f44f9]/5 rounded-full blur-3xl pointer-events-none">
            </div>
            <div class="flex items-center justify-between mb-6 border-b pb-4">
              <div class="flex items-center gap-2">
                <IconBallFootball />
                <h3 class="text-lg font-bold text--[#1c1c0d]">
                  Ficha Deportiva
                </h3>
              </div>
              <span class="text-xs font-medium px-2 py-1 bg-[#1f44f9]/20 text--[#1c1c0d] rounded-md">Perfil
                Jugador</span>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div class="md:col-span-3">
                <label class="flex flex-col gap-2">
                  ...
                  <div class="flex border rounded-xl border p-1 h-[50px]">
                    ...
                    <label class="cursor-pointer">
                      <input checked="" class="peer sr-only" name="foot" type="radio" value="right" />
                      <div
                        class="w-full h-full flex items-center justify-center rounded-lg text--white text-sm font-medium peer-checked:bg-white peer-checked:text--[#1c1c0d] peer-checked:shadow-sm transition-all">
                        Derecho
                      </div>
              ...
              <div class="md:col-span-12">
                <label class="flex flex-col gap-2">
                  <span class="text-sm font-semibold text--[#1c1c0d]">Notas Médicas / Observaciones</span>
                  <textarea
          ...
          <div class="flex gap-3 justify-end">
            <button class="px-5 py-2 rounded-lg border font-medium hover:bg-white transition-colors bg-transparent">
              Cancelar
            </button>
            <button type="submit"
